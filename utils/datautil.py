...
def load_test_data(dataset_name="huggingface", llm_name="CodeLlama-13b", init_alignment_ids=[], method="direct"):
    ...
    for line in open(gt_file, 'r'):
        ...
        if content["id"] in alignment_ids:
            ...
            # 【新增】加载GT的task_steps用于GNN评分
            pred_contents_dict[content["id"]]["gt_task_steps"] = reformat_steps(content)
...
def prepare_lm_gnn_training_data(dataset_name="huggingface", tmp_print=False, train_ids=None, maximum=''):
    ...
    for line in open(raw_data_file, 'r'):
        ...
        n_tools = content.get("n_tools")
        if n_tools is None:
            n_tools = len(content.get("task_nodes", []))
        content_type = content.get("type", "chain")
        ...
        elif n_tools > 1 and content_type == "chain":
            texts = content['task_steps']
...
def prepare_training_ids(dataset_name, train_num=3000, modes=['single', 'chain'], alignment_ids=None):
    ...
    for line in open(gt_data, 'r'):
        ...
        if content_type not in modes:
            continue
        if content_type == "chain":
            chain_ids.append(content["id"])
        else:
            single_ids.append(content["id"])

    random.shuffle(chain_ids)
    random.shuffle(single_ids)
    selected = (chain_ids + single_ids)[:train_num]
    chain_count = sum(1 for data_id in selected if data_id in set(chain_ids))
